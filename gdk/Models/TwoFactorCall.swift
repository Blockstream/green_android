...
public extension TwoFactorCall {

    func resolve(connected: @escaping() -> Bool = { true }, chain: String? = nil) -> Promise<[String: Any]> {
        func step() -> Promise<[String: Any]> {
            let bgq = DispatchQueue.global(qos: .background)
            return Guarantee().map(on: bgq) {
                try self.getStatus()!
            }.then { json in
                try self.resolving(json: json, connected: connected, chain: chain).map { _ in json }
    ...
    private func resolving(json: [String: Any], connected: @escaping() -> Bool = { true }, chain: String?) throws -> Promise<Void> {
        guard let status = json["status"] as? String else { throw GaError.GenericError() }
        let bgq = DispatchQueue.global(qos: .background)
        switch status {
        case "done":
