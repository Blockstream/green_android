...
struct Event: EventProtocol, Equatable {
    ...
    func title() -> String {
        if kindOf(TransactionEvent.self) {
            ...
            guard let twoFactorReset = SessionManager.shared.notificationManager.twoFactorReset else { return "" }
    ...
    func description(wallets: [WalletItem], twoFactorConfig: TwoFactorConfig?) -> String {
        if kindOf(TransactionEvent.self) {
            ...
            let btc = AccountsManager.shared.current?.gdkNetwork?.getFeeAsset() ?? ""
            ...
            if twoFactorConfig == nil {
                return ""
            } else if !twoFactorConfig!.anyEnabled {
