...
class TransactionStatusTableCell: UITableViewCell {
    ...
    func configure(for transaction: Transaction, isLiquid: Bool) {
        let blockHeight = NotificationManager.shared.blockHeight
        ...
        let showBumpFee = !isLiquid && transaction.canRBF && !isWatchonly && !(NotificationManager.shared.twoFactorReset?.isResetActive ?? false)
