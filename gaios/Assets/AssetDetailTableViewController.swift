    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if let cell = tableView.dequeueReusableCell(withIdentifier: "AssetDetailTableCell") as? AssetDetailTableCell {
            ...
            case .name:
                ...
                cell.detailLabel.text = isLBTC ? "Liquid Bitcoin" : asset?.name ?? NSLocalizedString("id_no_registered_name_for_this", comment: "")
            case .identifier:
            ...
            case .precision:
                ...
                cell.detailLabel.text = isLBTC ? "8" : String(asset?.precision ?? 0)
            case .ticker:
                cell.titleLabel.text = NSLocalizedString("id_ticker", comment: "")
                cell.detailLabel.text = isLBTC ? "L-BTC" : asset?.ticker ?? NSLocalizedString("id_no_registered_ticker_for_this", comment: "")
            case .issuer:
                cell.titleLabel.text = NSLocalizedString("id_issuer", comment: "")
                cell.detailLabel.text = isLBTC ? NSLocalizedString("id_lbtc_has_no_issuer_and_is", comment: "") : asset?.entity?.domain ?? NSLocalizedString("id_unknown", comment: "")
            }
            return cell
        }
